...
<header>
    ...
    <section>
        ...
        <p>El sueño es fundamental para la recuperación muscular y el bienestar mental. Durante el sueño profundo, el cuerpo libera >
<h2>Practicar la higiene del sueño</h2>
        <p>Para mejorar la calidad del sueño, se recomienda evitar el uso de pantallas al menos una hora antes de dormir, mantener u>
        <h2>Tomar pausas activas durante el día</h2>
        <p>Las pausas activas ayudan a reducir la fatiga y mejorar la concentración. Se recomienda levantarse, estirarse o caminar c>
        <h2>Escuchar a tu cuerpo y evitar el sobreentrenamiento</h2>
        <p>Es importante reconocer las señales de fatiga y permitir que el cuerpo descanse cuando sea necesario. Entrenar sin el des>    <footer>
...
# Ruta absoluta a tu HTML
launch(){
	HTML_PATH="$HOME/FitOS/fitos_desktop_ui.html"

# Verifica que el archivo existe
	if [ -f "$HTML_PATH" ]; then
  		echo "🌐 Abriendo FitOS UI en el navegador..."
  		firefox "$HTML_PATH"
	else
  		echo "❌ No se encontró el archivo HTML en: $HTML_PATH"
	fi
}
# Abrir ayuda en Lynx
salud_y_bienestar() {
